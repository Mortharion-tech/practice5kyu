...
Task
Given two positive integers start and limit, the function buddy(start, limit) should return the first pair (n m) of buddy pairs such that n (positive integer) is between
...
function buddy(start, limit) {
    for (let n = start; n <= limit; n++) {
        ...
        if (m > n) {
            const sumM = sumProperDivisors(m);
            if (sumM === n + 1) {
                return [n, m];
            }
        }
