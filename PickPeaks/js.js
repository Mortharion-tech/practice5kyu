...
function pickPeaks(arr) {
    ...
    for (let i = 1; i < arr.length - 1; i++) {
      ...
      if (platStart !== -1 && arr[i] > arr[i+1]) {
        result.pos.push(platStart);
        result.peaks.push(arr[platStart]);
        platStart = -1;
      }
      if (arr[i] < arr[i+1]) {
        platStart = i + 1;
...
/* function pickPeaks(arr) {
    const pos = arr.map((x,i)=>i > 0 ? Math.sign(x - arr[i-1]) * i : 0).filter(i => i != 0).filter((x,i,a) => i < a.length-1 && (a[i+1] < 0 && x > 0));
