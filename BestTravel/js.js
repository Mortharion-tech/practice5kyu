...
function chooseBestSum(t, k, ls) {
    ...
    const validCombinations = combinations(ls, k).filter(comb => comb.reduce((sum, val) => sum + val, 0) <= t);
    if (validCombinations.length === 0) return null;
    const bestSum = Math.max(...validCombinations.map(comb => comb.reduce((sum, val) => sum + val, 0)));
    return bestSum;
}

/* 
const chooseBestSum = (t, k, ls) => ls.reduce((pre, val) => [...pre, ...pre.filter(val => val.length < k).map(v => [...v, val])], [[]]).filter(val => val.length === k).map(val => val.reduce((pre, val) => pre + val)).filter(val => val <= t).sort((a, b) => a - b).pop() || null;
 */